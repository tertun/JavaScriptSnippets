...
#functions to test using kernels with selected regularization parameter
#ker: is any similarity matrix (not necessary centralized)
KerRegSel.LOF <- function(y,covx,ker,lambda.seq=10^seq(-5,5,by=1))
{
  n <- length(y)
  p <- ncol(covx)
  h <- diag(n)-matrix(1,n,n)/n
  ...
  J <- matrix(1,n,n)
  ker <- ker-J%*%ker0/(n-1)-ker0%*%J/(n-1)+J%*%ker0%*%J/n/(n-1) #centralized kernel
  shift.reg <- rep(NA,length(lambda.seq))
  for (k in 1:length(lambda.seq))
  {
    lambda <- lambda.seq[k]
    ker.reg <- ker-ker%*%solve(n*lambda*diag(n)+ker)%*%ker
    shift.reg[k] <- kerUTest(y,covx,ker.reg)$shift
  }
  lambda.best <- lambda.seq[which.max(shift.reg)]
  #ker.reg <- ker-ker%*%solve(n*lambda.best*diag(n)+ker)%*%ker
  return(lambda.best)
}

KerRegSel <- function(y,covx,ker,lambda.seq=10^seq(-5,5,by=1),L=5)
{
  #check and load packages
  list.of.packages <- c("psych","caret")
  new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
  ...
  n <- length(y)
  p <- ncol(covx)
  flds <- createFolds(y, k = L, list = TRUE, returnTrain = FALSE)
  lambda.sel.list <- rep(NA,L)
  for (l in 1:L)
  {
    set.out <- flds[[l]]
    y.LOF <- y[-set.out]
    covx.LOF <- covx[-set.out]
    ker.LOF <- ker[-set.out,-set.out]
    lambda.sel.list[l] <- KerRegSel.LOF(y.LOF,covx.LOF,ker.LOF,lambda.seq)
  }
  lambda.stab.sel <- median(lambda.sel.list)
  #ker.reg <- ker-ker%*%solve(n*lambda.stab.sel*diag(n)+ker)%*%ker
  #result <- kerUTest(y,covx,ker.reg)
  return(list(lambda.stab.sel=lambda.stab.sel,lambda.sel.list=lambda.sel.list))
}
